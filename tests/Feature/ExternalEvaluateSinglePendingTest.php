...
it('showEvaluation: ?focus={id} → ฟอร์มรายคน แม้เหลือหลายคน (โหมดคลิกการ์ด)', function () {
    ...
    // chain เต็ม: คลิกการ์ด = select-evaluatee single=1 → redirect evaluate?focus → form รายคน
    $this->post(route('external.select-evaluatee', ['evaluateeId' => $B->id]), ['single' => 1])
        ->assertRedirect(route('external.evaluate', ['focus' => $B->id]));
    $this->followingRedirects()
        ->post(route('external.select-evaluatee', ['evaluateeId' => $B->id]), ['single' => 1])
        ->assertInertia(fn (AssertableInertia $p) => $p->component('ExternalEvaluation')->has('evaluatees', 1)->where('evaluatees.0.id', $B->id));
    // ประเมินทั้งหมด (single=0) → ไม่ focus → หลายคน
    $this->followingRedirects()
        ->post(route('external.select-evaluatee', ['evaluateeId' => $A->id]), ['single' => 0])
        ->assertInertia(fn (AssertableInertia $p) => $p->has('evaluatees', 2));
    // JS เก่า cache (ไม่ส่ง single) → default focus → รายคน (กันเคสที่ผู้ใช้เจอ)
    $this->post(route('external.select-evaluatee', ['evaluateeId' => $B->id]))
        ->assertRedirect(route('external.evaluate', ['focus' => $B->id]));
});
