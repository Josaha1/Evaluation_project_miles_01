...
class EvaluationExportService
{
            ...
            if (empty($evaluationIds)) {
                ...
                foreach ($evaluationIds as $evalId) {
                    ...
                    $this->buildPivotSheet(
                        ...
                        true    // includeOpenText — รวม open_text เป็นคอลัมน์
                    );
                }
            }
    ...
    public function exportByEvaluationType(int $evaluationId, array $filters = []): string
    {
        try {
            ...
            // Use shared pivot helper — รวม open_text เป็นคอลัมน์ในชีต
            $this->buildPivotSheet($sheet, $evaluationId, $filters, null, false, false, true);
     ...
     */
    private function buildPivotSheet($sheet, int $evaluationId, array $filters = [], ?string $customTitle = null, bool $selfEvalOnly = false, bool $externalOrgMode = false, bool $includeOpenText = false): void
    {
        ...
        $questions = DB::table('questions as q')
            ...
            ->join('parts as p', 'asp.part_id', '=', 'p.id')
            ->where('p.evaluation_id', $evaluationId)
            ->whereIn('q.type', $includeOpenText ? ['rating', 'choice', 'multiple_choice', 'open_text'] : ['rating', 'choice', 'multiple_choice'])
            ->orderBy('p.order')->orderBy('asp.id')->orderBy('q.order')
        ...
        if ($externalOrgMode) {
            ...
            $query->join('users as evaluator', 'a.user_id', '=', 'evaluator.id')
                  ->leftJoin('positions as evaluator_pos', 'evaluator.position_id', '=', 'evaluator_pos.id')
                  ->join('evaluation_assignments as ea', function ($j) {
                      $j->on('a.evaluation_id', '=', 'ea.evaluation_id')
                        ->on('a.user_id', '=', 'ea.evaluator_id')
                        ->on('a.evaluatee_id', '=', 'ea.evaluatee_id');
            ...
            $query->addSelect([
                'evaluator.emid as evaluator_emid',
                DB::raw("CONCAT(evaluator.prename, evaluator.fname, ' ', evaluator.lname) as evaluator_name"),
                'evaluator_pos.title as evaluator_position',
            ]);
        }
        ...
        foreach ($results as $r) {
            ...
            if ($qIdx !== null) {
                ...
                } elseif ($qType === 'open_text') {
                    // open_text: ลงข้อความที่ตอบ (a.value) เป็น text cell เหมือน multiple_choice
                    $rows[$key]['scores'][$qIdx] = is_string($r->raw_value ?? null) ? $r->raw_value : '';
                } else {
                    $rows[$key]['scores'][$qIdx] = $r->score;
                }
            }
        ...
        if ($externalOrgMode) {
            ...
            $fixedHeaders = ['A5'=>'ลำดับ','B5'=>'รหัสผู้ถูกประเมิน','C5'=>'ชื่อผู้ถูกประเมิน','D5'=>'ระดับ','E5'=>'สายงาน','F5'=>'ฝ่าย','G5'=>'ตำแหน่ง','H5'=>'ผู้ประเมิน','I5'=>'ตำแหน่งของผู้ประเมิน','J5'=>'องศาการประเมิน'];
        }
        foreach ($fixedHeaders as $cell => $h) $sheet->setCellValue($cell, $h);
        ...
        foreach ($colLayout as $colIdx => $info) {
            ...
            if ($info['type'] === 'q') {
                ...
                $sheet->getColumnDimension($c)->setWidth(in_array($q->type, ['multiple_choice', 'open_text']) ? 40 : 6);
            } else {
                $sheet->setCellValue($c . '5', 'เฉลี่ย');
                $sheet->getColumnDimension($c)->setWidth(8);
            }
        ...
        foreach ($rows as $r) {
            ...
            if ($externalOrgMode) {
                ...
                $sheet->setCellValue('I' . $rowNum, $r['evaluator_position']);
        ...
        // Auto-width fixed columns (external +I,J · regular +I ตำแหน่งผู้ประเมิน · self เดิม)
        $autoCols = $externalOrgMode ? ['A','B','C','D','E','F','G','H','I','J'] : ($selfEvalOnly ? ['A','B','C','D','E','F','G','H'] : ['A','B','C','D','E','F','G','H','I']);
    ...
    public function exportSelfEvaluationReport(array $filters = []): string
    {
        try {
            ...
            foreach ($selfEvals as $eval) {
                ...
                $this->buildPivotSheet($sheet, $eval->id, $filters, 'รายงานการประเมินตนเอง: ' . $eval->title, true, false, true);
            }
