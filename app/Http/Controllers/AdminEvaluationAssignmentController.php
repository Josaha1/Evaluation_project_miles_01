...
class AdminEvaluationAssignmentController extends Controller
{
    ...
    /**
     * เปลี่ยนแบบประเมินของ assignment เดียว (ไม่แตะ evaluator/evaluatee/angle)
     */
    ...
    /**
     * เปลี่ยนแบบประเมิน bulk — all-or-nothing rollback ถ้ามี row ใด invalid
     */
    public function bulkChangeEvaluation(Request $request)
    {
        ...
        foreach ($rows as $row) {
            ...
            if ($err) {
                $errors[] = ['assignment_id' => $row->id, 'reason' => $err];
            }
        }
    ...
    /**
     * ตรวจสิทธิ์เปลี่ยน form — return string error หรือ null ถ้า OK
     */
    private function validateChangeEvaluation(EvaluationAssignment $assignment, int $newEvaluationId): ?string
    {
        if ($assignment->submitted_at !== null) {
            return 'เปลี่ยนแบบประเมินไม่ได้ — assignment นี้ถูกส่งแล้ว';
        }

        $eval = Evaluation::find($newEvaluationId);
        if (! $eval) {
            return 'ไม่พบแบบประเมินปลายทาง';
        }

        if ($eval->status !== 'published') {
            return 'แบบประเมินปลายทางยังไม่ published';
        }

        if ((string) $eval->fiscal_year !== (string) $assignment->fiscal_year) {
            return 'ปีงบประมาณของแบบประเมินไม่ตรงกับ assignment';
        }
