...
export default function AssignedEvaluationStep() {
    ...
    const groupedQuestions: Group[] = current_part.aspects.flatMap((aspect) => {
        ...
        const aspectOnly = aspect.questions.length > 0
            ? [{
                subaspectName: null,
    ...
    const handleNextGroup = async () => {
        ...
        if (Object.keys(currentAnswers).length > 0) {
            try {
                await axios.post(route('assigned-evaluations.step', {
            ...
            case 'rating':
                return (
                    <div className="flex justify-center gap-2">
                        {[5, 4, 3, 2, 1].map((val) => (
                            <Button key={val} variant={isSelected(val) ? 'default' : 'outline'} className="w-12 h-12 rounded-full" onClick={() => updateAnswer(question.id, val)}>
                                {val}
                            </Button>
                        ))}
                    </div>
